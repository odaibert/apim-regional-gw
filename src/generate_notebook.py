...
This interactive notebook demonstrates geographic routing with Azure Traffic Manager
and API Management.
Learn how to route API requests to the nearest regional gateway.
...
First, we'll deploy our FastAPI application to multiple regions using Azure App Service.
...
TRAFFIC_MANAGER_NAME = "apim-geo-demo-tm"
...
print(f"Traffic Manager endpoint: {traffic_manager_endpoint}")"""),
    ...
    eastus_id = !az webapp show \\
        --name apim-demo-eastus \\
        --resource-group {RESOURCE_GROUP} \\
        --query id -o tsv
    brazilsouth_id = !az webapp show \\
        --name apim-demo-brazilsouth \\
        --resource-group {RESOURCE_GROUP} \\
        --query id -o tsv
    ...
    !az network traffic-manager endpoint create \\
        ...
        --resource-group {RESOURCE_GROUP} \\
        --type azureEndpoints \\
        --target-resource-id {brazilsouth_id[0]} \\
        --geo-mapping GEO-SA \\
        --endpoint-status Enabled
